...
const SupplierSpendLineChart = () => {
  ...
  const groupByMonth = (data, year = new Date().getFullYear()) => {
    ...
    data.forEach((item) => {
      ...
      if (dateStr) {
        ...
        if (keyYear === year) {
          counts[key]["value"] += 1;
          counts[key]["spend"] += +item.vendor.supplierSpentOn;
        }
      }
    });
  ...
  let completedAudits = selectedData.filter(
    (item) =>
      item.auditorAssignmentSubmission &&
      item.auditorAssignmentSubmission.type == 2
  );

  const totalSpendOfAllSuppliers = selectedData.reduce(
    (acc, curr) => acc + curr.vendor.supplierSpentOn,
    0
  );
  const totalSpendOfCompletedSuppliers = completedAudits.reduce(
    (acc, curr) => acc + curr.vendor.supplierSpentOn,
    0
  );

  
  let monthsGrouping = groupByMonth(completedAudits);
  let trend = calculateTrend(monthsGrouping);
  let lineSeries = Object.keys(monthsGrouping).map((item) => {
    return {
      name: monthsGrouping[item]["label"],
      y: +monthsGrouping[item]["spend"].toFixed(2),
    };
  });
  ...
  const item = {
    ...
    value: `${
      (totalSpendOfCompletedSuppliers / totalSpendOfAllSuppliers).toFixed(1) * 100
    }%`,
    trend: trend,
        ...
        point: {
          events: {
            click: function () {
              ...
              const filtered = completedAudits.filter((item) => {
...
export default SupplierSpendLineChart;
